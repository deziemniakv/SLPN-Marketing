...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('dodaj-wyplate')
        .setDescription('Dodaje ręczny bonus BC do wypłaty pracownika (Dyrektor).')
        .addUserOption(option =>
            option.setName('pracownik').setDescription('Pracownik').setRequired(true)
        )
        .addIntegerOption(option =>
            option.setName('ilosc').setDescription('Ile BC dodać').setRequired(true).setMinValue(1)
        )
        .addStringOption(option =>
            option.setName('powod').setDescription('Powód bonusu (opcjonalnie)').setRequired(false)
        ),
